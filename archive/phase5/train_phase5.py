...
def parse_args(argv):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Skip s3 processing for HF bands (Option 3)"
    )
    parser.add_argument(
        "--use_simple_weconv",
        action="store_true",
        default=True,
        help="Use simple WeConv blocks (faster)"
    )    parser.add_argument(
        "--use_adaptive_quant",
        action="store_true",
        default=True,
        help="Use adaptive quantization (Phase 4 feature)"
    )
    parser.add_argument(
        "--attention_heads",
        type=int,
        default=8,
        help="Number of attention heads for frequency fusion"
    )
    parser.add_argument(
        "--num_checkerboard_groups",
        type=int,
        default=4,
        choices=[2, 4],
        help="Number of checkerboard groups (2 or 4, Phase 5 feature)"
    )
    parser.add_argument(
        "--use_enhanced_context",
        action="store_true",
        default=True,
        help="Use enhanced spatial context network (Phase 5 feature)"
    )
    parser.add_argument(
        "--lightweight_context",
        action="store_true",
        default=False,
        help="Use lightweight context networks for faster inference"
    )    parser.add_argument(
...
def main(argv):
    ...
    # Phase 5: Import HPCM_Checkerboard from current directory
    from HPCM_Checkerboard import HPCM_Checkerboard
    net = HPCM_Checkerboard(
        use_simple_weconv=args.use_simple_weconv,
        skip_s3_for_hf=args.skip_s3_for_hf,
        use_adaptive_quant=args.use_adaptive_quant,
        attention_heads=args.attention_heads,
        num_checkerboard_groups=args.num_checkerboard_groups,
        use_enhanced_context=args.use_enhanced_context,
        lightweight_context=args.lightweight_context
    )
    print(net)
    net = net.to(device)
